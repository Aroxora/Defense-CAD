...
Verifies that Type 052D destroyer models are properly integrated with the
defense contractor registry and CAD framework.

Tests include:
- Contractor registry integration (CSSC)
...
class TestType052DContractorIntegration(unittest.TestCase):
    """Test Type 052D integration with defense contractor registry"""

    def setUp(self):
        """Initialize contractor registry"""
        self.registry = DefenseContractorRegistry()

    def test_cssc_contractor_exists(self):
        """Test CSSC contractor is registered"""
        cssc = self.registry.get_contractor("CSSC")

        self.assertIsNotNone(cssc, "CSSC contractor should be registered")
        self.assertEqual(cssc.name, "CSSC")
        self.assertEqual(cssc.full_name, "China State Shipbuilding Corporation")
        self.assertEqual(cssc.country, "China")
        self.assertIn("Type 052D", cssc.operational_systems)
        self.assertGreaterEqual(cssc.confidence, 0.50)

    def test_type052d_model_registered(self):
        """Test Type 052D baseline model is registered"""
        model = self.registry.get_model("Type052D")

        self.assertIsNotNone(model, "Type 052D model should be registered")
        self.assertEqual(model.model_name, "Type052DModel")
        self.assertEqual(model.contractor, "CSSC")
        self.assertEqual(model.platform_name, "Type 052D Destroyer (Luyang III)")
        self.assertEqual(model.fielded_date, 2014)
        self.assertEqual(model.integration_level, "platform")

    def test_type052d_enhanced_model_registered(self):
        """Test Type 052D enhanced model is registered"""
        model = self.registry.get_model("Type052D_Enhanced")

        self.assertIsNotNone(model, "Type 052D Enhanced model should be registered")
        self.assertEqual(model.contractor, "CSSC")
        self.assertEqual(model.fielded_date, 2026)

    def test_chinese_naval_models(self):
        """Test all Chinese naval models are retrievable"""
        chinese_models = self.registry.get_models_by_country("China")

        naval_models = [m for m in chinese_models if "052D" in m.platform_name]
        self.assertGreaterEqual(
            len(naval_models), 2,
            "Should have at least 2 Type 052D variants"
        )
...
def run_integration_demonstration():
    ...
    # Test contractor registry integration
    print("1. CONTRACTOR REGISTRY INTEGRATION")
    print("-" * 80)
    registry = DefenseContractorRegistry()
    cssc = registry.get_contractor("CSSC")
    print(f"Contractor: {cssc.full_name}")
    print(f"Country: {cssc.country}")
    print(f"Operational Systems: {', '.join(cssc.operational_systems)}")
    print()

    # Test model retrieval
    print("2. MODEL RETRIEVAL")
    print("-" * 80)
    models = [
        registry.get_model("Type052D"),
        registry.get_model("Type052D_Enhanced")
    ]
    for model in models:
        print(f"{model.platform_name}:")
        print(f"  Fielded: {model.fielded_date}")
        print(f"  Confidence: {model.confidence:.0%}")
        print(f"  Integration Level: {model.integration_level}")
    print()

    # Test CAD capabilities
    print("3. CAD CAPABILITIES DEMONSTRATION")
    print("-" * 80)
    destroyer = Type052DModel(variant=Type052DVariant.ENHANCED)
