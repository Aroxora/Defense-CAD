...
class PhysicalOpticsRCSCalculator:
    ...
    def calculate_rcs(self,
        ...
        # Calculate RCS.
        # The reflection coefficient (gamma) is already applied to the scattered-field
        # amplitude inside the coherent sum above, so |complex_sum|^2 already carries the
        # gamma^2 power factor. Do NOT multiply by gamma^2 again here (that would make the
        # material attenuation scale as gamma^4 and grossly over-credit RAM).
        if visible_count > 0:
