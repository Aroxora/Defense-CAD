...
class OperationalRFPropagation:
    ...
    def _atmospheric_gas_absorption(self,
        ...
        # Sea-level approximation consistent with ITU-R P.676 (r_p = r_t = 1). Below the
        # 60 GHz O2 complex this is the P.676 Annex-2 dry-air form; the second term grows as
        # f -> 54 GHz to approach the band edge. Around 60 GHz the absorption is dominated by
        # the overlapping O2 lines (peak ~15 dB/km at sea level), modeled here as a Lorentzian
        # bump so the curve is continuous and physically realistic instead of a step jump.
        # Sanity: ~0.009 dB/km at 15 GHz (Ku), ~0.7 dB/km at 50 GHz, ~15 dB/km at 60 GHz.
        if f < 54.0:
            gamma_o = (7.2 / (f**2 + 0.34) + 0.62 / ((54.0 - f)**1.16 + 0.83)) * f**2 * 1e-3
        elif f < 63.0:
            gamma_o = 0.5 + 14.5 / (1.0 + ((f - 60.0) / 1.5)**2)  # 60 GHz O2 line, peak ~15
        else:
            gamma_o = 0.5 + 6.0 / (1.0 + ((f - 63.0) / 9.0)**2)   # decaying tail above the line

        # --- Water-vapor specific attenuation, dB/km ---
        # Compact P.676-consistent fit including the 22.235 GHz line (rho in g/m³).
        # Sanity at rho = 7.5 g/m³: ~0.02 dB/km at 15 GHz, ~0.18 dB/km at the 22 GHz line.
