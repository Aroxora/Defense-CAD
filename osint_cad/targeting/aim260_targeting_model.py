...
class AIM260TargetingModel:
    ...
    def _can_seeker_acquire(self, target_rcs_m2: float, range_km: float) -> bool:
        ...
        # Seeker detection range scales as RCS^0.25 (radar range equation, fourth-power law:
        # R_max ∝ (Pt G^2 λ^2 σ / ((4π)^3 k T B F SNR_min L))^(1/4), so R ∝ σ^(1/4)).
        # Signal processing improves the *constant* (lower achievable SNR_min via integration
        # gain) — it cannot change the σ^(1/4) exponent. Any seeker-sensitivity advantage is
        # already captured in reference_range / seeker_sensitivity_dbsm, not in the exponent.
        scaled_range = reference_range * (target_rcs_m2 / reference_rcs) ** 0.25 if target_rcs_m2 > 0 else 0.0
