...
try:
    ...
    from osint_cad.physics.rcs_models import F35ARCSModel, J20RCSModel
    from osint_cad.engagements.integrated_kill_chain_cad import IntegratedKillChainCAD
    from osint_cad.engagements.f35_defensive_vs_pl15 import F35DefensiveModel, DefensiveManeuver
    from osint_cad.engagements.information_chain_robustness import InformationChainValidator
...
class IntegratedKillChainSimulation:
    ...
    - EW (radar jamming; opportunistic MADL degradation -- see calculate_ew_effects
      for why reliable MADL jamming is not physically achievable from standoff)
    """
    ...
    def calculate_ew_effects(
        ...
        # MADL degradation -- OPPORTUNISTIC ONLY, NOT a reliable kill-chain step.
        #
        # MADL is a Ku-band, electronically-steered, narrow-beam (a few degrees) LPI/LPD
        # link with frequency hopping and low power spectral density. Effective jamming
        # from a standoff jammer is physically hard because:
        #   1) Geometry: energy must enter the narrow main/near-sidelobe of a link beam
        #      pointed between two F-35s. A standoff jammer is almost never on that axis,
        #      so it competes only against far sidelobes -- huge spatial-isolation penalty.
        #   2) Processing gain: the spread/hopped waveform rejects in-band narrowband
        #      jamming by the despreading gain (tens of dB) the jammer must also overcome.
        #   3) Frequency agility: hopping across the band means a fixed-offset jammer
        #      dwells on the link only a small fraction of the time.
        # We therefore model only a small, geometry-gated degradation and apply a large
        # spatial-isolation + processing-gain penalty (>= 25 dB) to J/S. This is a best
        # case for the jammer (assumes favorable geometry); typical effect is ~0.
        madl_isolation_db = 25.0  # spatial isolation (sidelobe) + residual processing gain
        madl_j_s_db = j_s_db - madl_isolation_db
        madl_effectiveness = effectiveness * 0.1 if madl_j_s_db > 10 else 0.0
        effects.append(EWEffect(
            target_system="MADL Network (opportunistic, geometry-dependent)",
            effect_type="degradation",
            effectiveness_percent=madl_effectiveness,
            jamming_power_kw=jam_power_kw * 0.5,  # Split power
            j_s_ratio_db=madl_j_s_db
        ))
    ...
    def generate_report(self, results: List[EngagementResult]) -> str:
        ...
        report.append("=" * 90)
        report.append("")
        report.append("1. DETECTION ADVANTAGE (geometry-dependent, not guaranteed)")
        report.append("   - J-20 ESM may intercept F-35 MADL sidelobes at 180-220 km, but")
        report.append("     ONLY when the link beam geometry and an active MADL transmission")
        report.append("     coincide; against a directional, frequency-hopping LPI link this")
        report.append("     is opportunistic, intermittent, and bearing-only without a")
        report.append("     synchronized multi-platform TDOA/FDOA network.")
        report.append("   - F-35 must radiate to detect J-20 at ~90-120 km")
        report.append("   - First-shot advantage IF intercept succeeds: +60-100 km")
        report.append("")
        ...
        report.append("   - MADL network: not reliably jammable from standoff (narrow")
        report.append("     steered beams + frequency hopping + processing gain). Any")
        report.append("     degradation is opportunistic and geometry-limited, not a")
        report.append("     dependable means of isolating F-35s.")
